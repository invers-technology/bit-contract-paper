...
We formalize the escrow problem in anonymous markets and derive requirements from two design goals:
\textbf{(Goal A)} mediation without transferring asset-transfer authority to the operator, and
\textbf{(Goal B)} smart-contract realization with minimal third-party trust by avoiding complex VM semantics and limiting the audit surface.

\subsection{Requirements}
From the goals above, we derive the following requirements.

\textbf{A1 (Non-custodial mediation).} During mediation, the operator alone must not be able to transfer assets to any terminal payout state.

\textbf{A2 (No unilateral settlement by a single party).} No single participant (buyer or seller) can unilaterally force a terminal transfer that contradicts the agreed workflow.

\textbf{A3 (No permanent freezing / liveness).} Locked assets must not remain indefinitely frozen; a timeout-based path must guarantee eventual progress to a terminal state.

\paragraph{Clarification on non-finalization.}
Non-finalization does not imply that funds remain permanently locked without recourse.
Even if the operator does not issue \textsf{FinalizeTx}, any contractual party can still finalize the workflow by issuing the corresponding terminal transaction (e.g., release/refund or a timeout-exit when enabled).
Therefore, the system does not rely on operator activity for safety or liveness: the contract state may remain observable as ``unfinalized,'' but the principal is not irreversibly frozen.

\paragraph{Liveness under fee volatility.}
A constant finalization reward can fail to incentivize \textsf{FinalizeTx} when network fees exceed the fixed reward, leading to indefinite non-finalization (operationally, lingering \textsf{FeeReserve}) in practice.
Therefore, the design includes a value-dependent incentive $b(V)$ with bounds and a bounded fee budget $F_{\max}$ reserved from the operator-side fee budget.

\textbf{B1 (Bounded verification and minimal third-party trust).} Contract behavior should avoid general-purpose VM execution. Verification must be bounded and auditable by construction using a UTXO-based transaction model and readable, template-constrained predicates (computational-law style validation).

\paragraph{Formalization mapping.}
We will express A1 and A2 as reachability constraints over the contract state machine (e.g., $\mathrm{Disc}(\cdot)=\emptyset$ for single-actor discretionary terminal transitions), A3 as a liveness theorem enabled by timeout conditions, and B1 as a design constraint on the execution model (templates over UTXO, no global mutable state, bounded validation).
...
運営が関与する仲裁遷移は存在するが、それらは常に $(\mathsf{pk}_A \text{ and } \mathsf{pk}_B)$ の署名を伴うため、裁量は「運営単独」から「当事者の同意の証跡」へ移送される。
