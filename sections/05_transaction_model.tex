...
将来拡張性の観点では、\texttt{proto\_version} と \texttt{args} により新型の追加を互換的に扱える。

\subsection{Aux（args）の規範仕様と合意対象}
本稿では、トランザクション補助データ \texttt{aux} を \textbf{canonicalized された TLV 列}として扱い、少なくとも \texttt{proto\_version}、\texttt{tx\_type}、\texttt{cid}、\texttt{args} を含むものとして規範化する。\texttt{args} は\textbf{型固有データ}であり、トランザクション種別ごとの入力を収容する。

合意・検証・\texttt{cid} 導出に用いる入力は、常に \textbf{シリアライズ済み TLV（bytes）}に固定する。例えば \texttt{init\_params\_bytes} のような \textit{bytes 表現}が合意対象であり、文字列表現（例：JSONや可読文字列）は表示・ログ用途の \textbf{非合意データ}に限定する。

また、\texttt{tx\_type} を合意対象として明示的に固定し、\texttt{tx\_type} $\in \{\mathrm{CREATE}, \mathrm{LOCK}, \mathrm{EXECUTE}, \mathrm{FINALIZE}\}$ とする。これは、型の曖昧解釈を排除して安価に分類できるようにし、DoS 耐性を高めると同時に、\texttt{proto\_version} と \texttt{args} によって将来拡張を可能にするためである。
...
The contract instantiation MUST create a dedicated \textsf{FeeReserve} output of value $R(V)$, funded from the operator fee budget.
This output is later consumed by \textsf{FinalizeTx} to reward the finalizer and to cover a bounded transaction fee.
...
本稿では (i) \texttt{tx\_type=LOCK} とし、\texttt{args} に create 情報（\texttt{init\_params\_bytes} 等）を含める方式を採用案とする。
