...
\end{equation}
ここで $v$ は額面、$u$ は通常 UTXO、$u^\star$ はテンプレート参照と状態タグを伴う Locked UTXO である。
\textbf{実装上は、$(\mathsf{cid}, \mathsf{stateTag})$ は script / aux / commitment / metadata に埋め込まれ得る抽象化であり、UTXO に独立の data 欄があることを仮定しない。}
（Bitcoin Script 相当では $(\mathsf{cid}, \mathsf{stateTag})$ を script/commitment/metadata として埋め込める。eUTXO では datum として扱える。）
...
\end{equation}
とする。$\mathsf{in}$ は入力 outpoint 列、$\mathsf{out}$ は出力 UTXO 列、$\mathsf{aux}$ は署名・期限情報など検証に必要な補助入力である。
...
\begin{itemize}
	...
	\item $\mathsf{Logic}$：検証ロジック（validator）。$\mathsf{Pred}$ の実装に相当する。
...
\end{equation}
で表す。$\vec{a}=(a_1,\dots,a_k)$ はテンプレートが定める初期化引数列であり、当事者集合（例：Buyer/Seller/Operator）や必要情報は $\vec{a}$ に含まれる前提とするため、$\mathsf{participants}$ のような別フィールドは置かない。
$\mathsf{cid}$ は決定的に導出される識別子であり、例えば
\begin{equation}
	\mathsf{cid} := H(\mathsf{deployerAddr} \parallel \mathsf{tid} \parallel \vec{a})
\end{equation}
...
Locked UTXO $u^\star$（Definition 1）において、$\mathsf{cid}$ は参照先インスタンス、$\mathsf{stateTag}$ はテンプレートが定める有限個の状態ラベル（例：$S0, S0D, S0X$）である。本稿では $\mathsf{stateTag}$ を共有される唯一のグローバルステートとして扱う。
...
\begin{itemize}
	\item CreateSC：$\Gamma$ 内のテンプレート $\mathsf{tid}$ と引数 $\vec{a}$、および $\mathsf{deployerAddr}$ を参照し、$\mathsf{cid}$ を生成するトランザクション。
