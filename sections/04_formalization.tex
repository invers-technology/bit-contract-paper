...
必要であればテンプレート集合や参照情報は immutable global registryとして保持し得る。
...
$\mathsf{Addr}$ はアドレス集合であり、$\mathsf{addr} \in \mathsf{Addr}$ は P2PKH などの通常アドレスや script-hash 的な識別子を抽象化したものである（実装上は script/commitment/metadata に埋め込める）。
...
\end{equation}
ここで $v$ は額面、$\mathsf{addr}$ は所有者（支出権限）を表すアドレス、$u$ は通常 UTXO、$u^\star$ はテンプレート参照と状態タグを伴うロック状態の UTXO である。
\textbf{実装上は、$(\mathsf{c}_{id}, \mathsf{stateTag})$ はトランザクションの $\mathsf{aux}$ に埋め込まれ得る抽象化であり、UTXO に独立のフィールドがあることを仮定しない。}
...
\end{equation}
とする。$\mathsf{in}$ は入力 outpoint 列 $\mathsf{in}:=[\mathsf{op}_j]$、$\mathsf{out}$ は出力 UTXO 列、$\mathsf{aux}$ は署名・期限情報など検証に必要な補助入力である。
Bitcoinと同様に、本稿のトランザクションは from/to アドレスを明示的なフィールドとして含まない。
本稿で $\mathsf{addr}_{\mathrm{from}}$ と記す場合は、入力 UTXO の所有者（消費権限の主体）を指す略記であり、明示フィールドではなく入力の検証から同定される。
...
を含む。ここで \texttt{payload\_bytes} は型固有データである。合意・検証・$\mathsf{c}_{id}$ 導出に用いるのは必ず byte[]（例：\texttt{init\_params\_bytes}）であり、JSON などの文字列表現は表示・ログ用途の非合意データに限る。
...
（例：Buyer/Seller/Operator）や必要情報は $\vec{a}$ に含まれる前提とするため、$\mathsf{participants}$ のような別フィールドは置かない。
コントラクトインスタンスの内部変数（$\mathsf{InitParams}$）は、CreateContract によって生成された際から不変である。
$\mathsf{c}_{id}$ は決定的に導出される識別子であり、例えば
\begin{equation}
	\mathsf{c}_{id} := H(\mathsf{tx}_{id} \parallel \mathsf{addr}_{\mathrm{from}} \parallel \mathsf{nonce} \parallel \mathsf{t}_{index} \parallel \mathsf{init\_params\_bytes})
\end{equation}
のように作れる（重要なのは同一入力から一意であること）。
ここで $\mathsf{tx}_{id}$ は CreateContract トランザクションの ID、$\mathsf{addr}_{\mathrm{from}}$ は CreateContract/LockUtxo を作成する主体の \textbf{from（送信元）} アドレス（入力 UTXO の所有者アドレス）であり、$\mathsf{nonce}$ は衝突回避のための一意化要素である。
これにより「参照先インスタンスが曖昧」「後から差し替え」等の裁量余地を縮退する（R2, R3）。

\paragraph{Definition 8：$u^\star$：型付き状態}
$u^\star$ は、単なる script 断片ではなくテンプレート参照と状態タグを伴う型付き状態である、という設計選好を仕様として固定する。
...
\begin{itemize}
	\item CreateContract：$\Gamma$ 内のテンプレート $\mathsf{t}_{index}$ と \texttt{init\_params\_bytes}、および $\mathsf{tx}_{id}$・$\mathsf{addr}_{\mathrm{from}}$・$\mathsf{nonce}$ を参照し、$\mathsf{c}_{id}$ を生成するトランザクションである。ContractTx 共通ヘッダに従い、$\mathsf{aux}$ の \texttt{tx\_type} は \texttt{CREATE} とする。
	\item LockUtxo：通常 UTXO $u$ を消費し、$u^\star$ を出力する。LockUtxo はインスタンス化された $\mathsf{c}_{id}$ に対して送信され、$\mathsf{stateTag}$ を初期状態に持つ $u^\star$ を生成する。出力には $(\mathsf{c}_{id}, \mathsf{stateTag}=s_{\mathrm{init}})$ が付与される。ここで $s_{\mathrm{init}} \in \mathsf{StateSet}_\tau$ はテンプレートが定める初期状態である。ContractTx 共通ヘッダに従い、$\mathsf{aux}$ の \texttt{tx\_type} は \texttt{LOCK} とする。
\end{itemize}

\paragraph{Definition 10：Release / Transition}
$u^\star$ を入力として消費するトランザクションを contract transition $tx$ と呼ぶ。
...
つまり、ExecuteContract トランザクションは $u^\star$ の状態遷移を命令するものであり、テンプレートはその検証機として働く。
