...
本稿で扱う Contract Transaction の $\mathsf{aux}$ は canonical encoding された byte 列とし、少なくとも
\texttt{proto\_version}、
\texttt{tx\_type} $\in \{\mathrm{CREATE},\mathrm{LOCK},\mathrm{EXECUTE},\mathrm{FINALIZE}\}$（FINALIZE は任意）、
\texttt{cid}、
\texttt{schema\_id} または \texttt{t\_index}、
...
すなわち、$\mathsf{aux}$ 内の \texttt{proto\_version}/\texttt{tx\_type}/\texttt{payload\_bytes}（必要に応じて \texttt{cid}/\texttt{schema\_id} or \texttt{t\_index}）は canonical TLV bytes として一意に解釈される。
...
cid/署名/検証の合意対象は canonical TLV bytes であり、例として \texttt{init\_params\_bytes} は canonical TLV bytes として扱う。
