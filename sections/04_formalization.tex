...
\end{equation}
\begin{equation}
	u^\star=(\mathsf{op}, v, \mathsf{c}_{id}, \mathsf{stateTag})
\end{equation}
...
\end{equation}
とする。
$U$ は未消費 UTXO の集合、$\Gamma$ はテンプレート集合（および必要な参照情報）から成る不変レジストリである（B1）。
...
本稿で扱う Contract Transaction の $\mathsf{aux}$ は canonical encoding された byte 列とし、先頭に共通ヘッダ \texttt{proto\_version}、\texttt{codec\_id}、\texttt{tx\_type} $\in \{\mathrm{CREATE},\mathrm{LOCK},\mathrm{EXECUTE},\mathrm{FINALIZE}\}$、\texttt{payload\_len}、\texttt{cid} を置き、続く \texttt{payload} に型固有データを格納する。
ここで \texttt{payload} は型固有データである。
合意・検証・$\mathsf{c}_{id}$ 導出に用いるのは必ずシリアライズ済み byte[]（例：\texttt{init\_params\_bytes}）であり、JSON などの文字列表現は表示・ログ用途の非合意データに限る。
\texttt{tx\_type} を合意対象として固定するのは、型の曖昧解釈を排除し、安価な分類で DoS 耐性を高め、\texttt{proto\_version}/\texttt{codec\_id} により将来拡張を可能にするためである。
$\mathsf{aux}$ は合意対象データ（consensus-critical bytes）であり、Contract Transaction の wire-level encoding は共通ヘッダ＋\texttt{payload}（TLV）として規範化する。
すなわち、$\mathsf{aux}$ 内の \texttt{payload} は canonical TLV bytes として一意に解釈される。
...
規範仕様は次のとおりである。
\begin{itemize}
	\item Tag（Type）：u16 の big-endian
	\item Length：u32 の big-endian
	\item Value：raw bytes
	\item 要素順序：Tag 昇順でソート
	\item 重複 Tag：禁止（duplicate tag は invalid）
	\item 未知 Tag：strict では reject（将来拡張は \texttt{proto\_version}/\texttt{codec\_id} で扱う）
\end{itemize}
...
\begin{itemize}
	...
	\item $\mathsf{TransSpec}$：状態と生成する $tx$ の形の二つから成る出力仕様（finalization incentive は定数 $b$ ではなく、取引額に依存する $b(V)$ として扱う）
\end{itemize}
本稿ではテンプレート $\tau$ を記述する DSL として Computational Law DSL である Stipula を採用する。
Stipula は自然言語的な契約条項を $\mathsf{Pred}$ および各遷移に対応する $\mathsf{TransSpec}$ として形式化し、検証者が $tx$ を見て一意に判定できる決定的条件（B1）として与える。
また、テンプレート制約による監査上限（B1）を、実装規律ではなく仕様記述の表現力として固定できる点が Stipula 採用の理由である。
...
コントラクト・インスタンス $I$ は
\begin{equation}
	I := (\mathsf{c}_{id}, \mathsf{t}_{index}, \mathsf{pk}_{\mathrm{deployer}}, \vec{a})
\end{equation}
で表す。
$\mathsf{pk}_{\mathrm{deployer}}$ はインスタンスを生成した deployer（作成者）の公開鍵であり、CreateContract においては $\mathsf{pk}_{\mathrm{from}}$ と一致する。
...
\begin{equation}
	\mathsf{c}_{id} := H(\mathsf{tx}_{id} \parallel \mathsf{pk}_{\mathrm{deployer}} \parallel \mathsf{t}_{index} \parallel \mathsf{init\_params\_bytes})
\end{equation}
のように作れる（重要なのは同一入力から一意であること）。
ここで $\mathsf{tx}_{id}$ は CreateContract $tx$ の ID、$\mathsf{pk}_{\mathrm{deployer}}$ は CreateContract/LockUtxo を作成する主体の \textbf{from（送信元）} 公開鍵（入力 $u$ の所有者公開鍵）である。
これにより「参照先インスタンスが曖昧」「後から差し替え」等の裁量余地を縮退する（B1）。
...
要件は「外部裁量ではなく検証可能」＝決定的 admissibility である（B1）。
...
\subsection{Finalization incentive $b(V)$ and fee reserve}
Let $V$ denote the principal amount locked in escrow (the value transferred between contractual parties, excluding operator fees).
We define the finalization incentive $b(V)$ as a bounded proportional function:
\[
	b(V) := \min\left(b_{\max},\; \max\left(b_{\min},\; \alpha_b \cdot V\right)\right),
\]
where $\alpha_b \in (0,1)$ is the incentive rate, and $b_{\min}, b_{\max} > 0$ are lower/upper bounds.

To ensure liveness under variable network fees, we additionally reserve a bounded fee budget $F_{\max} \ge 0$.
We define the \emph{fee reserve} amount as:
\[
	R(V) := b(V) + F_{\max}.
\]

\paragraph{Design principle.}
The principal $V$ MUST NOT be reduced to fund finalization.
Instead, $R(V)$ is deducted from the operator-side fee budget (or explicitly provisioned operator reserve), so that the contract parties' settlement amounts remain unchanged while finalization remains incentivized.
...
\subsection{不変条件（Invariants）と要件 A1--A3, B1 への対応}
本稿は、資金拘束・解放を「人間の裁量」ではなく制度（検証規則）として固定し、失敗遷移（misappropriation）を排除／拘束することを狙う。
以下、テンプレート型スマートコントラクトが満たすべき不変条件を定義し、A1--A3, B1 と対応付ける（RQ3）。
...
Operator が単独で資金を不正流用できる遷移 $F$ が admissible にならない（A1）。
...
各テンプレート $\tau$ について、検証者が点検すべきロジックは $\mathsf{Bound}$ により上限付けられ、インスタンス化により複雑性が無制限に増えない（B1）。

\paragraph{要件対応（まとめ）}
\begin{itemize}
	\item A1：Operator 単独での終端資金移動（misappropriation / 任意解放）を admissibility から排除（Authority Safety）
	\item A2：単独裁量による終端決着を排除し、合意または期限条件により遷移を拘束（reachability / Disc）
	\item A3：期限条件により Locked 状態が永久残留しない（Liveness）
	\item B1：$\mathsf{Pred}$/$\mathsf{TransSpec}$ をテンプレートとして制約し、検証量と監査対象を上限化
	      （UTXO 上の局所状態、共有語彙は $\mathsf{stateTag}$、参照情報 $\Gamma$ は不変）
\end{itemize}
