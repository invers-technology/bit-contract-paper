...
ただし $\Gamma$ の変更（追加・廃止・修正）は アプリ層での差し替えではなく、ネットワーク合意を伴うプロトコル更新（soft/hard fork）としてのみ起こり得る。
本稿は単一バージョン $\mathsf{v}_{\mathrm{proto}}$ に固定して議論する。
...
\begin{itemize}
	\item CreateContract：$\Gamma$ 内のテンプレート $\mathsf{t}_{index}$ と \texttt{init\_params\_bytes}、および $\mathsf{tx}_{id}$・$\mathsf{pk}_{\mathrm{from}}$・$\mathsf{nonce}$ を参照し、$\mathsf{c}_{id}$ を生成する $tx$ である。
ContractTx 共通ヘッダに従い、$\mathsf{aux}$ の \texttt{tx\_type} は \texttt{CREATE} とする。
...
ContractTx 共通ヘッダに従い、$\mathsf{aux}$ の \texttt{tx\_type} は \texttt{LOCK} とする。
\end{itemize}
