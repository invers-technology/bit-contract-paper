...
また、$\mathsf{pk}_{\mathrm{from}} = \mathsf{pk}_C$ とし、CreateContract の $\mathrm{txid}$ と $\mathrm{nonce}$ による一意化を含めて cid を導出し、例えば
\[
	\mathrm{cid} := H(\mathrm{txid} \parallel \mathsf{pk}_{\mathrm{from}} \parallel \mathrm{nonce} \parallel \mathrm{tindex} \parallel \mathrm{init\_params\_bytes})
\]
...
\begin{table*}[t]
	...
	\begin{tabularx}{\textwidth}{l l l l l X}
		...
		$\sigma_A$                                                     &
		$S0$                                                           &
		$p$ を拘束し $u^\star(\mathrm{cid},\mathrm{stateTag}{=}S0)$ を生成                                                           \\
		...
		$\sigma_A$                                                     &
		$S1$                                                           &
		$p(1-m-b)\rightarrow \mathsf{pk}_B,\; pm\rightarrow \mathsf{pk}_C,\; pb\rightarrow \mathsf{pk}_A$（bounty は Buyer に帰属） \\
		...
		$S1$                                                           &
		$p(1-m-b)\rightarrow \mathsf{pk}_B,\; pm\rightarrow \mathsf{pk}_C,\; pb\rightarrow \mathsf{pk}_F$（bounty は起動者に帰属）     \\
...
さらに FinalizeContract は誰でも実行可能で bounty（$pb$）が起動者 $\mathsf{pk}_F$ に帰属するため、期限到来後に実行されやすい。
