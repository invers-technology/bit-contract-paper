...
また、$\mathsf{pk}_{\mathrm{from}} = \mathsf{pk}_C$ とし、CreateContract の $\mathsf{tx}_{id}$ により cid を導出し、例えば
\[
	\mathrm{cid} := H(\mathsf{tx}_{id} \parallel \mathsf{pk}_{\mathrm{from}} \parallel \mathsf{t}_{index} \parallel \mathrm{init\_params\_bytes})
\]
